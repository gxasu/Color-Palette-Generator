...
## ADR-019: OKLCH パレットの初期明度カーブを -30 に固定

**日付**: 2026-04-20
**ステータス**: 採用
**コンテキスト**: 新規 OKLCH パレット作成時と、インポートデータに `lightnessCurve` が存在しない場合のフォールバックが `0.3` になっており、UI 要件のデフォルト `-30` とずれていた。
**決定**: OKLCH パレットの初期 `lightnessCurve` は `-0.3` に統一する。新規作成、生成関数の既定値、インポート時の欠損フォールバックをすべて同じ値に揃える。
**根拠**: デフォルト値の出どころを複数箇所で一致させることで、初回表示・追加作成・旧データ読込のどの経路でも同じカーブ挙動になるため。

---

## ADR-018: テーマメニューの状態同期を render サイクルへ統合

**日付**: 2026-04-20
**ステータス**: 採用
**コンテキスト**: テーマ切り替え UI を `select` からカスタムドロップダウンへ置き換えるにあたり、トリガーアイコン、選択状態、`data-theme` の適用が分散すると再描画との整合が崩れやすい。
**決定**: テーマ適用 (`applyTheme`) とトリガー/選択肢の状態同期 (`syncThemeControls`) は `render(state)` の中で毎回実行する。クリックイベント側は `setTheme()` のみを責務にする。
**根拠**: このアプリは状態変更ごとに全体再レンダリングする設計であり、テーマ UI も同じ一方向フローへ寄せるほうが再構築時のズレや二重同期を避けやすい。
...
## ADR-015: Feather Icons をローカル依存として同梱

**日付**: 2026-04-20
**ステータス**: 採用
**コンテキスト**: `md-icon` は Material Symbols のフォント読込に依存しており、フォント未到達時にリガチャ文字列がそのまま UI に露出した。PWA としてオフライン寄りに振る舞うアプリでは、外部フォント依存は壊れやすい。
**決定**: `feather-icons` を npm 依存として追加し、`data-feather` プレースホルダを `feather.replace()` で SVG に変換する方式へ移行。Google Fonts の Material Symbols 読み込みと `@material/web/icon/icon.js` 依存は削除した。
**根拠**:
- アイコンが SVG としてバンドルされるため、外部フォント CDN への依存がなくなる
- `slot="icon"` を保持したまま `md-filled-tonal-button` / `md-icon-button` に差し込める
- アプリの全体再レンダリングに合わせて都度 `feather.replace()` を呼ぶだけで整合が取れ、既存描画構造を崩さず導入できる

---

## ADR-016: npm overrides による監査対象依存の固定

**日付**: 2026-04-20
**ステータス**: 採用
**コンテキスト**: `npm audit` で `vite <= 6.4.1` に加えて、その配下の `rollup` と `picomatch` に high severity が報告された。`vite` を 6.4.2 に上げても lockfile 上は脆弱な下位依存が残存した。
**決定**: `vite` を `6.4.2` へ更新し、さらに `package.json` の `overrides` で `rollup` を `^4.60.2`、`picomatch` を `^4.0.4` に固定した。
**根拠**:
- 既存の Vite 6 系を維持しつつ、最小変更で high severity を解消できる
- transitive dependency のみを安全版へ引き上げられ、アプリコードへの影響が小さい
- `npm audit` を 0 件まで落とせることを確認済み

---

## ADR-017: パレット追加アクションをトリガー + メニューに集約

**日付**: 2026-04-20
**ステータス**: 採用
**コンテキスト**: 左パネル上部で通常パレット追加と Alpha パレット追加を別ボタンとして常時表示していたため、狭い幅の中で見出しの改行と視覚的な混雑が起きていた。
**決定**: 静的ヘッダー領域に icon-only の追加トリガーを置き、クリック時のみメニューを開いて `createPalette()` と `createAlphaPalette()` を選択できる構成に変更した。メニューの開閉は `ui.js` のグローバルイベントで管理し、外側クリックと `Escape` で閉じるようにした。
**根拠**:
- DOM の再描画対象外である静的ヘッダーにメニュー状態を閉じ込められる
- パレット生成の入口を1か所に集約できる
- 左パネルの横幅を圧迫せず、見出しと操作の両立がしやすい
